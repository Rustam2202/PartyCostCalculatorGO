...
func (c *calculationTestCases) assertCheck(t *testing.T, result *EventData, err error) {
	...
	assert.EqualValues(t, c.testCase.exp.average, result.Average,
...
func TestRun(t *testing.T) {
	...
	for _, tc := range cases {
		c.testCase = tc()
		c.fillMock()
		result, err := c.serv.CalculateEvent(c.ctx, 1)
...
func testCase1() *testCase {
	var tc testCase
	tc.testName = "one debetor to two recepients"
...
func testCase2() *testCase {
	var tc testCase
	tc.testName = "two debetors to one recepient"
	tc.events = []domain.Event{
		{
			Id:      1,
			Name:    "New Year",
...
func testCase3() *testCase {
	var tc testCase
	tc.testName = "all spent without debts"
	tc.events = []domain.Event{
		{
			Id:      1,
			Name:    "New Year",
...
func testCase4() *testCase {
	var tc testCase
	tc.testName = "all spents equal zero"
	tc.events = []domain.Event{
		{
			Id:      1,
			Name:    "New Year",
